...
class mainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.setGeometry(300, 50, 995, 790)
        self.init_ui()

    def init_ui(self):
        # self.load_faild.connect(self.handle_load_faild)
        self.tabs = API.widgets.Tabs()  # create tabs
        ...
        AddNewTabKeyShortcut.activated.connect(
            lambda: self.add_new_tab(
                QtCore.QUrl(API.settings_data["newTabPage"], "New tab")
            )
        )
        ...
        self.reload_butn.setShortcut("F5")
        ...
        # Add Address bar
        self.url_bar = API.widgets.AddressBar()
        ...
        # current_url = str(self.tabs.currentWidget().page().title())
        ...
        self.httpsicon = API.widgets.SSLIcon()
        ...
        newTabAction.triggered.connect(
            lambda: self.add_new_tab(
                QUrl(API.settings_data["newTabPage"]), "Homepage"
            )
        )
        ...
        # Exit
        CloseBrowser = QAction("Exit", self)
        ...
        if self.check_internet_connection():
            ...
            self.add_new_tab(QUrl(API.settings_data["startupPage"]), "Homepage")
        else:
            
            self.add_new_tab(QUrl("http://127.0.0.1:5000"))
    ...
    def goToHome(self):
        self.tabs.currentWidget().setUrl(QUrl(API.settings_data["homeButtonPage"]))
    ...
    def visitGithub(self):
        self.add_new_tab(
            QUrl("https://github.com/Omarpython/"), "Github"
        )
    ...
    def reload_tab(self):
        ...
        current_url = str(self.tabs.currentWidget().page().url())
        one = current_url[19:-2]
        self.url_bar.setText(one)
    ...
    def open_local_file(self):
        ...
        if filename:
            try:
                ...
                dlg = API.errors.fileErrorDialog()
                dlg.exec_()
    ...
    def print_this_page(self):
        try:
            handler_print = API.printer.PrintHandler()
    ...
    def PrintWithPreview(self):
        handler = API.printer.PrintHandler()
    ...
    def tab_open_doubleclick(self, i):
        if i == -1:  # No tab under the click
            self.add_new_tab(QUrl(API.settings_data["newTabPage"]), label="New tab")
    ...
    def update_title(self, browser):
        ...
        if 0 > len(title):
            self.setWindowTitle("Internet MDF")
    ...
    def add_new_tab(self, qurl=None, label="Blank"):
        if qurl is None:
            qurl = QUrl(API.settings_data["newTabPage"])

        _browser = QWebEngineView()  # Define the main webview to browser the internet

        # Set page
        _browser.setPage(API.widgets.customWebEnginePage(_browser))
    ...
    def showErrorDlg(self):
        dlg = API.errors.errorMsg()
        dlg.exec_()

    def about(self):
        self.AboutDialogue = API.about.AboutDialog()
    ...
    def update_urlbar(self, q, _browser=None):
        ...
        if q.toString() == API.settings_data["newTabPage"]:
    ...
    def searchWeb(self, text):
        Engine = API.settings_data["defaultSearchEngine"]
        if text:
            if Engine == "JSTOR":
                return "https://www.jstor.org/action/" + "+".join(text.split())

            elif Engine == "Microsoft Academic":
                return "https://www.microsoft.com/" + "+".join(text.split())

            elif Engine == "Google Scholar":
                return "https://scholar.google.com/" + "+".join(text.split())

            elif Engine == "Base":
                return "https://www.base-search.net/Search/" + "+".join(text.split())
    ...
    def updateHistory(self):
        ...
        for i in range(len(siteInfoList)):
            if url == siteInfoList[i][2]:
                API.cursor.execute("DELETE FROM history WHERE url = ?", [url])

        tash_title = encrypt.encrypt(title, 1989)
        tash_url = encrypt.encrypt(url, 1989)
        tash_date = encrypt.encrypt(date, 1989)

        API.cursor.execute(
            "INSERT INTO history (title,url,date) VALUES (:title,:url,:date)",
            {"title": tash_title, "url": tash_url, "date": tash_date},
        )

        API.connection.commit()

    def openHistory(self):
        self.historyWindow = API.history.HistoryWindow()
    ...
    def openSettings(self):
        self.userSettingswindow = API.settings.UserSettings()
